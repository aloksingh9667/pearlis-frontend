  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok) {
          toast({ title: "Google Sign-Up Failed", description: data.error || "Could not authenticate with Google.", variant: "destructive" });
          return;
        }
        login(data.token);
        setStep("done");
    ...
    onError: () => {
      toast({ title: "Google Sign-Up Cancelled", description: "Please try again.", variant: "destructive" });
    },
  });
